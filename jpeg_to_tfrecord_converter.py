...
def persistence_image_data_to_tfrecords(x_data, y_data, data_type, split_index=128):
    ...
    for images_filename, y_label in zip(x_data, y_data):
        ...
        try:
            ...
            image_list = []
            boxconvert = iputil.get_bounding_size_info(y_label, images_filename)
            for size in boxconvert:
                xmin, ymin, bounding_width, bounding_height = size
                image = tf.image.crop_to_bounding_box(image, ymin, xmin, bounding_height, bounding_width)
